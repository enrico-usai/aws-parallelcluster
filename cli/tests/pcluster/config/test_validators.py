...
@pytest.mark.parametrize(
    ...
    [
        ("wrong_instance_type", "is not supported"),
        ("t2.micro", None),
        ("c4.xlarge", None),
        ("c5.xlarge", "is not supported"),  # to verify mock is working
    ],
)
def test_instance_type_validator(mocker, instance_type, expected_message):
    mocker.patch("pcluster.config.validators.get_supported_instance_types", return_value=["t2.micro", "c4.xlarge"])
